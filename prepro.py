...
def get_POS_one_hot_vector(word):

    if not word:
    	return np.zeros(37)

    pos_tags = nltk.pos_tag(word)[0][1]
    ...
    pos = ['SYM','SC','CC','CD','DT','EX','FW','IN','JJ','JJR','JJS','LS','MD','NN','NNS','NNP','NNPS','PDT','POS','PRP','PRP$','RB','RBR','RBS','RP','TO','UH','VB','VBD','VBG','VBN','VBP','VBZ','WDT','WP','WP$','WRB']
    ...
    if pos_tags in SC:
	pos_tags = 'SC'
    onehot_vector = np.zeros(len(pos))
    onehot_vector[onehot_dict[pos_tags]] = 1
    ...
    if emb_file is not None:
        ...
        with open(emb_file, "r", encoding="utf-8") as fh:
            for line in tqdm(fh, total=size):
                ...
                word = "".join(array[0:-(vec_size-37)])
		onehotvector = get_POS_one_hot_vector(word)

		#print(len(onehotvector))
                vector = list(map(float, array[-(vec_size-37):]))
		vector2 = list(map(float,np.array(onehotvector)))
		vector3 = vector+vector2
		#print(vector3)
		#print(len(vector),len(vector2),len(vector3))
                if word in counter and counter[word] > limit:
                    embedding_dict[word] = vector3
...
def build_features(config, examples, data_type, out_file, word2idx_dict, char2idx_dict, is_test=False):
    ...
    for example in tqdm(examples):
        ...
        pos_tags = nltk.pos_tag(example["ques_tokens"])
        noun_words = [tag[0] for tag in pos_tags if tag[1] in ["NN"]]
        ...
        for noun in noun_words:
            try:
                curr_extra_info = wiki.summary(noun,sentences=1)
            except:
                pass
     
            extra_info += curr_extra_info
            curr_extra_info = ''

        tokens = nltk.word_tokenize(extra_info)
        pos_of_tokens = nltk.pos_tag(tokens)
        
        nouns = [tag[0] for tag in pos_of_tokens if tag[1] in ["NN"]]
        for n in nouns:
            if len(example["ques_tokens"])<50:
                example["ques_tokens"].append(n)
