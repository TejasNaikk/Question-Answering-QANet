...
from tensorflow.python.ops.rnn import dynamic_rnn as _dynamic_rnn,bidirectional_dynamic_rnn as _bidirectional_dynamic_rnn
#  bidirectional_rnn as _bidirectional_rnn
...
def bidirlstm(fw_cell, bw_cell, inputs, d):
    print "in bidir"
    print inputs.shape
    # flat_inputs = flatten(inputs, 2)
    (fw, bw), _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=fw_cell, cell_bw=bw_cell, inputs=inputs,dtype=tf.float32)
    return ()
    # print "fw", fw_0.shape
    # print "bw", bw_0.shape    
    # fw_g = reconstruct(fw_0, inputs, 2)
    # bw_g = reconstruct(bw_0, inputs, 2)
    # print "fw_g0", fw_g.shape
    # print "bw_g0", bw_g.shape    
    # g = tf.concat([fw_g, bw_g], 1)
    # # g1 = tf.concat(3, [fw_g1, bw_g1])
    # # g0 = tf.concat([fw_0, bw_0], 3)
    # print "g", g.shape
    # print "returning from bidir"
    # return g
...
def drnn(cell, inputs, d):
    fw0, _ = dynamic_rnn(cell, inputs, dtype=tf.float32)
    return fw0
